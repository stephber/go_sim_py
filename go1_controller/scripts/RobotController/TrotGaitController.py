...
class TrotSwingController:
    ...
    def raibert_touchdown_location(self, leg_index, command):
        scale_factor = 1.0  # Увеличение длины шага на 50%
    ...
    def swing_height(self, swing_phase):
        scale_factor = 1.0  # Увеличение высоты на 20%
        if swing_phase < 0.5:
...
class TrotStanceController:
    ...
    def position_delta(self, leg_index, state, command):
        ...
        velocity = np.array([
            -(step_dist_x / 2) / (float(self.time_step) * self.stance_ticks),
            -(step_dist_y / 2) / (float(self.time_step) * self.stance_ticks),
